import { mysqlTable, mysqlSchema, AnyMySqlColumn, primaryKey, int, varchar, index, foreignKey, mysqlEnum, text, timestamp, datetime, char, tinyint } from "drizzle-orm/mysql-core"
...
(table) => [
	primaryKey({ columns: [table.id], name: "authors_id"}),
]);

export const bookAuthors = mysqlTable("book_authors", {
	bookId: int("book_id").notNull().references(() => books.id, { onDelete: "cascade" } ),
	authorId: int("author_id").notNull().references(() => authors.id, { onDelete: "cascade" } ),
},
(table) => [
	index("author_id").on(table.authorId),
	primaryKey({ columns: [table.bookId, table.authorId], name: "book_authors_book_id_author_id"}),
]);
...
(table) => [
	...
	primaryKey({ columns: [table.bookId, table.categoryId], name: "book_categories_book_id_category_id"}),
]);
...
export const bookCopies = mysqlTable("book_copies", {
	...
	status: mysqlEnum(['available','checked_out','reserved','lost','maintenance']).default('available').notNull(),
},
(table) => [
...
export const books = mysqlTable("books", {
	...
	publisherId: int("publisher_id").references(() => publishers.id, { onDelete: "set null" } ),
	publicationYear: int("publication_year"),
	languageId: int("language_id").references(() => languages.id, { onDelete: "set null" } ),
	description: text(),
...
export const borrowTransactions = mysqlTable("borrow_transactions", {
	id: int().autoincrement().notNull(),
	copyId: int("copy_id").notNull().references(() => bookCopies.id, { onDelete: "cascade" } ),
	userId: varchar("user_id", { length: 255 }).notNull(),
	borrowDate: datetime("borrow_date", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`).notNull(),
...
export const cartItems = mysqlTable("cart_items", {
	userId: varchar("user_id", { length: 255 }).notNull(),
	bookId: int("book_id").notNull().references(() => books.id, { onDelete: "cascade" } ),
	addedAt: datetime("added_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`).notNull(),
},
(table) => [
	index("book_id").on(table.bookId),
	primaryKey({ columns: [table.userId, table.bookId], name: "cart_items_user_id_book_id"}),
]);
...
(table) => [
	primaryKey({ columns: [table.id], name: "publishers_id"}),
]);
...
export const reservations = mysqlTable("reservations", {
	...
	userId: varchar("user_id", { length: 255 }).notNull(),
	bookId: int("book_id").notNull().references(() => books.id, { onDelete: "cascade" } ),
	reservedAt: datetime("reserved_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`).notNull(),
	status: mysqlEnum(['active','fulfilled','canceled']).default('active').notNull(),
...
(table) => [
	index("book_id").on(table.bookId),
	primaryKey({ columns: [table.userId, table.bookId], name: "wishlists_user_id_book_id"}),
]);
